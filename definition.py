...
def segment_by_spend_level(customer_data):
    ...
    """
    # Define bins up to the desired ranges
    bins = [0, 100, 500, 1000, 1500, customer_data['total_spent'].max()]  # Adjusted bins
    labels = [
        'Low (<100)', 
        'Moderate (100-500)', 
        'High (500-1,000)', 
    ...
    fig.update_layout(yaxis_type='log', yaxis_title='Customer Count (Log Scale)')
    return fig
...
def segment_by_order_frequency(customer_data):
    ...
    customer_data['order_frequency'] = pd.cut(
        ...
        bins=[0, 1, 5, 10, 20, customer_data['total_orders'].max()],
        labels=['1 Order', '2-5 Orders', '6-10 Orders', '11-20 Orders', '21+ Orders']
    )
...
def visualize_customer_distribution_city(customer_data, world_cities):
    # Normalize city names and country codes to lowercase for consistent matching
    ...
    # Filter world cities to only include ISO2 codes present in the customer data
    iso2_list = customer_data['iso2'].unique()
    filtered_cities = world_cities[world_cities['iso2'].isin(iso2_list)]
    if filtered_cities.empty:
        print(f"No matching cities found for the given ISO2 codes: {iso2_list}. Please check your data.")
        return
    # Merge customer data with the filtered cities
    df_customer_merged = pd.merge(
        customer_data,
        filtered_cities,
    ...
    if df_customer_merged.empty:
        print("No matching data after merging customer data with city data. Please check your inputs.")
        return
    # Aggregate customer data by city
    city_summary = df_customer_merged.groupby(['city', 'lat', 'lng']).agg(
    ...
    if city_summary.empty:
        ...
        return
    # Center the map on the country or default to global view
    map_center = {
        'lat': city_summary['lat'].mean() if not city_summary.empty else 0,
        'lon': city_summary['lng'].mean() if not city_summary.empty else 0
    }
    ...
    radius=20,
    hover_name='city',
    hover_data=['total_customers', 'total_spent'],
    mapbox_style='carto-positron',
    title='Customer Distribution Heatmap',
    color_continuous_scale=px.colors.sequential.Agsunset,  # Use Reds for color scale
    center=map_center,
    zoom=5
    )
    return fig
